...
(function ($) { // hide the namespace
    ...
    $.extend(Datepick.prototype, {
        ...
        _connectDatepick: function (target, inst) {
            ...
            input.addClass(this.markerClassName).keydown(this._doKeyDown).keypress(this._doKeyPress).bind("setData.datepick", function (event, key, value) {
        ...
        _inlineDatepick: function (target, inst) {
            ...
            if (divSpan.hasClass(this.markerClassName))
                return;
            divSpan.addClass(this.markerClassName).bind("setData.datepick", function (event, key, value) {
                inst.settings[key] = value;
            }).bind("getData.datepick", function (event, key) {
                return this._get(inst, key);
            });
        ...
        _destroyDatepick: function (target) {
            ...
            if (nodeName == 'input') {
                $target.siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove().end().removeClass(this.markerClassName).unbind('focus', this._showDatepick).unbind('keydown', this._doKeyDown).unbind('keypress', this._doKeyPress);
        ...
        _updateDatepick: function (inst) {
            ...
            inst.dpDiv.empty().append(this._generateHTML(inst)).find('iframe.' + this._coverClass).css({
                width: dims.width,
                height: dims.height
            });
    ...
    $.fn.datepick = function (options) {
        ...
        if (typeof options == 'string' && (options == 'isDisabled' || options == 'getDate'))
            return $.datepick['_' + options + 'Datepick'].apply($.datepick, [this[0]].concat(otherArgs));
