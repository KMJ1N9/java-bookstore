    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#btnPay").click(function () {
                ...
                $.ajax({
                    url: "<c:url value='/order/pay'/>?oid=" + oid + "&_csrf_token=${CSRF_TOKEN}",
                    success: function (result) {
                        if (result == "success") {
                            alert("支付成功！");
                            // 支付成功后跳转到订单列表页面
                            window.location.href = "<c:url value='/order/myOrders'/>";
                        } else {
                            alert("支付失败！");
                        }
                    }
...
<div class="divContent">
    ...
    <span class="spanOid">订单号：${order.oid}</span>
</div>
<form action="" method="post" id="form1" target="_top">
    ...
    <div class="divBank">
        ...
        <div style="margin: 40px;">
            <a href="javascript:void(0);" id="btnPay" class="linkNext">支付</a>
        </div>
    </div>
</form>
